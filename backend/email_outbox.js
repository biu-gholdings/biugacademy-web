...
function buildSupportHtml(payload) {
  const rows = [
  ...
  const table = rows
    .map(([label, value]) => `<tr><td style="padding:8px 12px;font-weight:700;vertical-align:top">${escapeHtml(label)}</td><td style="padding:8px 12px">${escapeHtml(value || "—")}</td></tr>`)
    .join("");

  return `<!doctype html><html><body style="font-family:Arial,sans-serif;color:#161616"><h2>New BIU.G Academy application</h2><table style="border-collapse:collapse;width:100%;max-width:760px">${table}</table></body></html>`;
}
...
async function dispatchPendingEmails(pool, limit = 10) {
  ...
  try {
    ...
    const result = await client.query(
      `SELECT id, application_id, message_type, recipient_email, subject, payload, attempts
       FROM email_outbox
